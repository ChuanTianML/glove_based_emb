...
void *glove_thread(void *vid) {
    ...
    for(a = 0; a < lines_per_thread[id]; a++) {
        ...
        //fprintf(stderr, "glove_thread in for adaptive gradient.\n");
    ...
    //fprintf(stderr, "glove_thread end for.\n");
    if (fin) {
		//fprintf(stderr, "going to fclose(fin).\n"); 
		fclose(fin);
	}
    //fprintf(stderr, "glove_thread going to exit thread.\n");
    pthread_exit(NULL);
    //fprintf(stderr, "glove_thread exit thread.\n");
}
...
int train_glove() {
    ...
    //if(verbose > 0) fprintf(stderr,"going to start SGD.\n");
    ...
    for(b = 0; b < num_iter; b++) {
        ...
        //if(verbose > 0) fprintf(stderr,"going to join thread.\n");
        for (a = 0; a < num_threads; a++) {
			//fprintf(stderr, "start joining thread %lld\n", a); 
			pthread_join(pt[a], NULL); }
        //if(verbose > 0) fprintf(stderr,"going to add the cost.\n");
        for (a = 0; a < num_threads; a++) total_cost += cost[a];
        //if(verbose > 0) fprintf(stderr,"going to print iter result.\n");
